...
# Define the true values for the linear equation y = mx + c
true_m = 12.5
true_c = 25
...
# Build the linear model using Keras
...
# Get the learned values for m and c
m_learned = model.layers[0].get_weights()[0][0][0]
c_learned = model.layers[0].get_weights()[1][0]

# Print the original and learned parameters
print(f"\nOriginal m: {true_m:.2f}, c: {true_c:.2f}")
