    ...
    while queue:
        ...
        for neighbor_info in graph[current_node]:
            ...
            if neighbor not in visited:
                ...
                heapq.heappush(queue, (heuristic, neighbor, path + [neighbor], total_distance + distance))
    return None
...
def index():
    ...
    if start and goal and start in campus_graph and goal in campus_graph:
        results = [
            breadth_first_search(campus_graph, start, goal),
            depth_first_search(campus_graph, start, goal),
            uniform_cost_search(campus_graph, start, goal),
            euclidean_search(campus_graph, start, goal)
        ]
        result = min(results, key=lambda x: x['distance'] if x else float('inf'))
